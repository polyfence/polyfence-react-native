...
### Changed
- **polyfence-core bumped 1.0.11 → 1.0.13.** Picks up the new `applyRemoveZoneDirect` / `applyClearZonesDirect` companion helpers (see the Bug-021 fix below), and activates the `signalLost` / `signalRestored` events + `gpsStalenessTimeoutMs` config bridged in the Added section above — core 1.0.11 never emitted those events, so the bridge wiring stayed dormant until this bump.
