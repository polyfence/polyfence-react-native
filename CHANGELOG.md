...
### Documented
- **`removeZone()` and `clearAllZones()` on Android dispatch asynchronously (Bug-021).** Both methods route through a `startService` Intent to the `LocationTracker` foreground Service; the returned `Promise` resolves when the Intent has been queued, not when the Service has finished the removal. Consequence: `getZoneStates()` immediately after a removal on Android may still show the zone(s). iOS is synchronous — the tracker's removal runs to completion before the bridge Promise resolves. JSDoc on both methods now documents the asymmetry and the ~500ms read-after-write guidance. Behaviour is unchanged; only the documentation is new.
